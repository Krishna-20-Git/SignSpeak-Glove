...
class AppState extends ChangeNotifier {
  ...
  Future<void> _init() async {
    ...
    // Listen to incoming gesture IDs (e.g., "01", "05") from BleService
    _bleService.gestureStream.listen((id) {
      processGestureId(id);
    });
  }
  ...
  Future<void> _loadPreferences() async {
    ...
    _selectedLanguage = supportedLanguages.firstWhere((l) => l.code == langCode,
    ...
    for (final item in historyJson.reversed.take(50)) {
      try {
        _history.add(SpeechHistoryEntry.fromJson(jsonDecode(item)));
      } catch (_) {}
    }
    notifyListeners();
  }
  ...
  void processGestureId(String id) {
    final text = _gestureService.resolve(
        gestureId: id,
  ...
  void _speak(String text, String gestureId) {
    ...
    final entry = SpeechHistoryEntry(
        gestureId: gestureId,
        text: text,
        languageCode: _selectedLanguage.code,
  ...
  void repeatLast() {
    if (_lastText.isNotEmpty) {
      _ttsService.speak(_lastText, _selectedLanguage.code);
    }
  }
  ...
  Future<void> startBleScan() async {
    ...
    if (statuses[Permission.bluetoothScan]!.isGranted &&
      ...
      // 3. Call the actual Bluetooth logic in BleService
      ...
      // Handle the case where the user said no
      _bleScanning = false;
      notifyListeners();
